...
def normalize(mx):
    ...
    count = np.zeros((mx.shape[0],mx.shape[1]))
    count[mx.toarray() > 0] = 1
    rowsum = np.array(mx.sum(1))
    count = np.array(count.sum(1)).reshape((-1,1))
...
def load_citation(dataset_str="cora"):
    ...
    _, max_degree = normalize(adj)
    adj = sparse_mx_to_torch_sparse_tensor(adj)
    return adj, features, labels, idx_train, idx_val, idx_test, max_degree


def load_percent(dataset_str, per):
    names = ['x', 'y', 'tx', 'ty', 'allx', 'ally', 'graph']
    objects = []
    for i in range(len(names)):
        with open("../data/ind.{}.{}".format(dataset_str.lower(), names[i]), 'rb') as f:
    ...
    train_size=math.ceil(per/100*adj.shape[0]/labels.shape[1])
    print(dataset_str,'train_size',train_size)
    train_size = [train_size for i in range(labels.shape[1])]
    # np.random.shuffle(idx)
    tidx=np.arange(len(labels)-1500)
