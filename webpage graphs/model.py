...
class nof(nn.Module):
    def __init__(self, mydev, myf, max_degree, adj, gamma, nnode, nfeat, nlayers,nhidden, nclass, dropout, lamda, alpha, variant):
        ...
        for _ in range(nlayers):
            self.convs.append(noflayer(nnode, nhidden, nhidden, self.adj, self.max_degree, alpha, lamda, variant=variant))
    ...
    def forward(self, x):
        ...
        for i, con in enumerate(self.convs):
            ...
            #layer_inner = self.leakyrelu(con(layer_inner, _layers[0], self.lamda, self.alpha, i + 1))
            #_layers.append(layer_inner)
        layer_inner = F.dropout(layer_inner, self.dropout, training=self.training)
