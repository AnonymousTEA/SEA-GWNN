...
def build_and_train(hype_space,datastr, splitstr,i):
    adj, features, labels, idx_train, idx_val, idx_test, num_features, num_labels, deg_vec, raw_adj = full_load_data(datastr,splitstr)
...
for i in range(10):
    ...
    #params={"lr_rate_mul":1,"alpha":0.5,"lambda":1,"gamma":0.2,"wd1":0.05,"wd2":0.00001,"dropout":0.4,"hidden":64,"myf":1.2}
    params={"lr_rate_mul":1.2,"alpha":0.2,"lambda":0.5,"gamma":0.2,"wd1":0.05,"wd2":0.0005,"dropout":0.0,"hidden":64,"myf":1.2}
...
#np.save('train_idx list', idx_train_list)
#np.save('val_idx list', idx_val_list)
#np.save('test_idx list', idx_test_list)
